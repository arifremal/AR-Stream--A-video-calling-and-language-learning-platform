...
export async function signup(req, res) {
  ...
  try {
    ...
    const ranndomAvator = `https://avatar.iran.liara.run/public${idx}.png`;
    const newUser = await User.create({
      email,
      fullName,
      password,
      ProfilePic: ranndomAvator,
    });
    try {
      ...
      console.log(`stream new user crteated ${newUser.fullName}`);
    } catch (error) {
      console.log("error created in stream use", error);
    }
...
export async function login(req, res) {
  try {
    ...
    const isPasswrdCorrect = await user.matchPassword(password);
    if (!isPasswrdCorrect)
...
export async function onboard(req, res) {
  ...
  try {
    ...
    console.log(`stream user updated after onboardning for ${updatedUser.fullName}`)
  } catch (streamError) {
    ...
    console.error("Onboarding funnctionn error", error);
